    ...
    <div className="min-h-screen bg-background p-4 sm:p-6 flex items-center justify-center">
      <div className="w-full max-w-2xl space-y-4 sm:space-y-6">
        ...
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-4xl font-heading font-bold bg-gradient-primary bg-clip-text text-transparent">
            Choose Your Battle
          </h1>
          <p className="text-sm sm:text-base text-muted">Configure your 1v1 quiz match</p>
          <div className="inline-flex items-center gap-2 bg-card px-3 sm:px-4 py-2 rounded-lg border border-card-border">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                {categories.map((cat) => (
                  <Button
                    ...
                    className="h-16 sm:h-20 flex-col gap-1 sm:gap-2 hover:scale-105 transition-all duration-200"
                    onClick={() => setConfig({...config, category: cat.value})}
                  >
                    <span className="text-lg sm:text-2xl">{cat.icon}</span>
                    <span className="text-xs sm:text-sm font-medium text-center">{cat.label}</span>
                  </Button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {difficulties.map((diff) => (
                  ...
                  >
                    <span className={`font-medium text-sm sm:text-base ${diff.color}`}>{diff.label}</span>
                    <span className="text-xs text-muted-foreground hidden sm:block">{diff.description}</span>
                  </Button>
                ))}
              </div>
            </div>
                ...
                {matchLengths.map((length) => (
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-medium text-sm sm:text-base">{length.label}</span>
                    </div>
                    <span className="text-xs text-muted-foreground hidden sm:block">{length.duration}</span>
                  </Button>
                ))}
              </div>
            </div>
